...
- Open up a new python file (CC-decrypt.py) in IDLE and add the following lines of code.
#+BEGIN_SRC python
...
:HTML_CONTAINER_CLASS: design
:END:
- The algorithm for this isn't to complicated.
...
- For each of the =for= loops above, descrive what they print out.
...
- Let's go back to the /interpreter/ to try and see if we can reporduce the error.
#+begin_src python
myString = 'abcdefg'
myString[3]
myString[7]
